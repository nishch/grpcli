...
function getRequestPayload(dataFile) {
    try {
        if (!path.isAbsolute(dataFile)) {
            dataFile = path.join(process.cwd(), dataFile);
        }
...
function handleSlaveMessages(msg) {
    ...
    if ((msg.type === message.MESSAGE_TYPES.RESPONSE) || (msg.type === message.MESSAGE_TYPES.ERROR)) {
...
function run(options) {
    ...
    const { service, endpoint, method, dataFile, protoFile, number, wait, timeout, protoRoot } = options;

    _requestsCnt = number;
    let workersNum = number > 1 ? os.cpus().length : 1;

    createSlaves(workersNum, [endpoint, service, method, timeout, protoFile, protoRoot]);
