...
function makeCall(client, method, timeout) {
    ...
    return (request) => {
        return new Promise((resolve, reject) => {
            ...
            if (!request) {
                client[method](undefined, { deadline: now.setMilliseconds(now.getMilliseconds() + timeout) && now.getTime() }, cb);
                return;
            }

            client[method](request, { deadline: now.setMilliseconds(now.getMilliseconds() + timeout) && now.getTime() }, cb);
        });
    };
}
...
function execute() {
    ...
    let [endpoint, service, method, timeout, protoFile, protoRoot] = options;
    const pkg = getProtoPkg(protoFile, protoRoot, options);
    const client = createClient(pkg, service, endpoint, options);
    _makeCall = makeCall(client, method, timeout);
    registerWithParent();
}
